...
impl DocumentState {
    ...
    pub fn get_folding_ranges(&self) -> Vec<FoldingRange> {
        ...
        for block in self.get_graphql_trees() {
            ...
            Self::collect_folding_ranges(self, root, offset, &mut ranges);
        }
    ...
    fn collect_folding_ranges(
        ...
        if let Some(range) = Self::node_to_folding_range(doc, node, offset) {
            ranges.push(range);
        }
        ...
        for child in node.children(&mut cursor) {
            Self::collect_folding_ranges(doc, child, offset, ranges);
        }
    }

    fn node_to_folding_range(
        doc: &DocumentState,
        node: Node,
        offset: usize,
    ) -> Option<FoldingRange> {
        ...
        let file_range = doc.translate_to_file_range(node, offset);
        ...
        let (start_line, end_line) = match kind {
            ...
            | "input_object_type_extension" => {
                ...
                for child in node.children(&mut cursor) {
                    ...
                    {
                        let child_range = doc.translate_to_file_range(child, offset);
