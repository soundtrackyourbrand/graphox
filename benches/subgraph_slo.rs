...
fn bench_subgraph_lsp_actions(c: &mut Criterion) {
    ...
    let target_field = "field_0";
    let target_offset = schema_content.find(target_field).unwrap();
    let mut target_line = 0;
    let mut target_col = 0;
    for (i, c) in schema_content.char_indices() {
        if i >= target_offset {
            break;
        }
        if c == '\n' {
    ...
    let target_position = Position::new(target_line, target_col);
