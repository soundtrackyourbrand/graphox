...
pub async fn run_codegen(
    client: Client,
    config: Config,
    type_caches: Arc<dashmap::DashMap<String, Arc<graphql_codegen::TypeCache>, ahash::RandomState>>,
    ...
    let workspace_metadata = graphql_core::engine::Engine::scan_workspace(&config);
    ...
    for (project, project_meta) in config.projects.iter().zip(&workspace_metadata.projects) {
        ...
        let schema = match graphql_core::schema::load_schema(&config.base_dir, &project.schema) {
            Ok(s) => s,
            Err(e) => {
                client
        ...
        let project_context = graphql_core::engine::Engine::resolve_project_context(
            &valid_schema,
            global_metadata,
            project_files,
        );
        ...
        let type_cache = type_caches
            ...
            .or_insert_with(|| Arc::new(graphql_codegen::TypeCache::new()))
            .clone();
        ...
        for path in project_files {
            ...
            if let Some(doc) = workspace_metadata.documents.get(path) {
                ...
                let ctx = graphql_codegen::CodegenContext::new(
                    &valid_schema,
                ...
                if let Ok((ts_code, mut ops)) = graphql_codegen::generate_typescript(doc, &ctx) {
                    let out_path = graphql_core::utils::get_output_path(
                        path,
                        &config.base_dir,
                        project_output_dir,
                    );
