...
### 4. Integrated LSP
`graphox` isn't just a CLI tool; it's a Language Server. This means your IDE validation and your codegen are powered by the exact same engine.
...
### 2. No Plugin API
`graphql-codegen` allows users to write custom plugins in JavaScript to transform output. **`graphox` does not have a plugin API**, and there are currently no plans to implement one. The core is written in Rust for maximum performance, and all generation logic is built-in.
