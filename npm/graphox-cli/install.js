...
function getBinaryName() {
  return process.platform === 'win32' ? 'graphox.exe' : 'graphox';
}

function getDownloadURL(version) {
  const target = getPlatform();
  const ext = process.platform === 'win32' ? 'zip' : 'tar.gz';
  const baseURL = process.env.GRAPHOX_DOWNLOAD_URL || 
    'https://github.com/soundtrack/graphox/releases/download';
  
  return `${baseURL}/v${version}/graphox-${target}.${ext}`;
}
...
async function install() {
  try {
    ...
    // If GRAPHOX_LOCAL_BUILD is set, try to copy from local build
    const localBuildPath = process.env.GRAPHOX_LOCAL_BUILD;
    if (localBuildPath) {
    ...
    const ext = process.platform === 'win32' ? 'zip' : 'tar.gz';
    const archivePath = join(binDir, `graphox.${ext}`);
    ...
    console.error('https://github.com/soundtrack/graphox/releases');
    process.exit(1);
  }
}
