...
async fn test_lsp_smart_extract_fragment() {
    ...
    let response = service.call(Request::build("textDocument/codeAction").params(serde_json::to_value(params).unwrap()).id(1).finish()).await.unwrap().unwrap();
    ...
    if let CodeActionOrCommand::CodeAction(ca) = extract_action {
        ...
        let fragment_def_edit = file_changes.iter().find(|e| e.new_text.contains("fragment")).unwrap();
        assert!(fragment_def_edit.new_text.contains("on User"), "Fragment should be on type User, got: {}", fragment_def_edit.new_text);
    }
}
...
async fn test_lsp_smart_extract_field() {
    ...
    if let CodeActionOrCommand::CodeAction(ca) = extract_action {
        ...
        let fragment_def_edit = file_changes.iter().find(|e| e.new_text.contains("fragment")).unwrap();
        assert!(fragment_def_edit.new_text.contains("on Query"), "Fragment for field 'me' should be on type Query, got: {}", fragment_def_edit.new_text);
    }
}
