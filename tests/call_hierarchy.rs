...
async fn test_fragment_call_hierarchy() {
    ...
    let request = Request::build("textDocument/prepareCallHierarchy").id(1).params(serde_json::to_value(&params).unwrap()).finish();
    let response = service.call(request).await.unwrap().unwrap();
    let items: Option<Vec<CallHierarchyItem>> = serde_json::from_value(response.result().unwrap().clone()).unwrap();
    ...
    let request = Request::build("callHierarchy/incomingCalls").id(2).params(serde_json::to_value(&params).unwrap()).finish();
    let response = service.call(request).await.unwrap().unwrap();
    let incoming: Option<Vec<CallHierarchyIncomingCall>> = serde_json::from_value(response.result().unwrap().clone()).unwrap();
    ...
    let request = Request::build("textDocument/prepareCallHierarchy").id(3).params(serde_json::to_value(&params).unwrap()).finish();
    let response = service.call(request).await.unwrap().unwrap();
    let items: Vec<CallHierarchyItem> = serde_json::from_value(response.result().unwrap().clone()).unwrap();
    ...
    let request = Request::build("callHierarchy/outgoingCalls").id(4).params(serde_json::to_value(&params).unwrap()).finish();
    let response = service.call(request).await.unwrap().unwrap();
    let outgoing: Option<Vec<CallHierarchyOutgoingCall>> = serde_json::from_value(response.result().unwrap().clone()).unwrap();
