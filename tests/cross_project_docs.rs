...
async fn test_cross_project_docs_and_imports() {
    ...
    let item = items.iter().find(|i| i.label == "UserFields").expect("Should find UserFields");
    ...
    service.call(Request::build("textDocument/didChange").params(serde_json::to_value(DidChangeTextDocumentParams {
        text_document: VersionedTextDocumentIdentifier { uri: query_uri.clone(), version: 3 },
        content_changes: vec![TextDocumentContentChangeEvent {
            range: None,
            range_length: None,
    ...
    let request = Request::build("textDocument/completion").id(3).params(serde_json::to_value(&params).unwrap()).finish();
    let response = service.call(request).await.unwrap().unwrap();
    let result: Option<CompletionResponse> = serde_json::from_value(response.result().unwrap().clone()).unwrap();
    ...
    let item = items.iter().find(|i| i.label == "user").expect("Should find user");
    let doc = match item.documentation.as_ref().unwrap() {
        Documentation::MarkupContent(m) => &m.value,
        _ => panic!("Expected markup"),
    };
