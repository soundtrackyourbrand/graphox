...
async fn test_fragment_call_hierarchy() {
    ...
    let frag_text = r#"
        fragment User|Fields on User { id name }
        fragment UserWithHome on User { ...UserFields }
    "#;
    let (frag_text, position1) = with_cursor(frag_text);
    ...
    let params = CallHierarchyPrepareParams {
        text_document_position_params: TextDocumentPositionParams {
            ...
            position: position1, // UserFields definition
        },
    ...
    let (frag_text2, position2) = with_cursor(
        r#"
        fragment UserFields on User { id name }
        fragment User|WithHome on User { ...UserFields }
    "#,
    );
    let frag_uri2 = write_project_file(&dir, "fragments2.graphql", &frag_text2);
    lsp_did_open(&mut service, frag_uri2.clone(), "graphql", 1, &frag_text2).await;
    ...
    let params = CallHierarchyPrepareParams {
        text_document_position_params: TextDocumentPositionParams {
            ...
            position: position2, // UserWithHome definition
        },
        work_done_progress_params: Default::default(),
    };
...
async fn test_call_hierarchy_tsx() {
    ...
    let (frag_text, position) = with_cursor("fragment User|Fields on User { id }");
    let frag_uri = write_project_file(&dir, "fragments.graphql", &frag_text);
    lsp_did_open(&mut service, frag_uri.clone(), "graphql", 1, &frag_text).await;
