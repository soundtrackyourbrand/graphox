...
async fn test_fragment_completion_self_reference() {
    ...
    let (text, position) = with_cursor(
        "fragment OtherFrag on User { id }\nfragment UserFrag on User { ...| }\nquery { user { ...OtherFrag } }",
    );
    let uri = write_project_file(&dir, "test.graphql", &text);
    lsp_did_open(&mut service, uri.clone(), "graphql", 1, &text).await;
    ...
    assert!(
        ...
        "Expected fragment completion to exclude self-reference"
    );
    assert!(
        items.iter().any(|i| i.label == "OtherFrag"),
        "Expected other fragment suggestions to still be available"
    );
}
