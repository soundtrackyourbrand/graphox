...
async fn test_remove_first_duplicate() {
    ...
    let doc = create_doc(query_uri.as_str(), query_text);
    let diagnostic = Diagnostic {
        range: range_for_token_at_index(&doc, query_text, "id", 1),
    ...
    assert!(
        edits.iter().any(|e| e.new_text.is_empty()),
        "Should find an edit that removes the duplicate field"
    );
}
...
async fn test_type_only_fragment_used() {
    ...
    assert!(
        edits.iter().any(|e| e.new_text.is_empty()),
        "Should find an edit with empty new_text"
    );
}
