...
async fn test_references_embedded_directive() {
    let schema = "directive @skip(if: Boolean!) on FIELD | FRAGMENT_SPREAD | INLINE_FRAGMENT\ntype Query { user: User }\ntype User { id: ID! }";
    let (tmpdir, mut config) = make_temp_project_with_schema(schema, "**/*.{graphql,tsx}");
    config = config.with_base_dir(std::fs::canonicalize(tmpdir.path()).unwrap());
    let (mut service, _handle) = create_initialized_lsp_service(config).await;
...
async fn test_references_embedded_argument() {
    ...
    // Create TSX file with argument - cursor on argument name
    let tsx_text = r#"const query = graphql`query { user(|id: "1") { id } }`;"#;
    let (tsx_text, position) = with_cursor(tsx_text);
    let tsx_uri = write_project_file(&tmpdir, "query.tsx", &tsx_text);
    lsp_did_open(&mut service, tsx_uri.clone(), "typescript", 1, &tsx_text).await;
    ...
    let locations = result.expect("Expected locations for argument reference");
    assert!(
        !locations.is_empty(),
...
async fn test_references_embedded_cross_file() {
    ...
    // Create usage in .tsx file
    let usage_text = "const query = graphql`query { user { ...UserFields } }`;";
    let usage_uri = write_project_file(&tmpdir, "usage.tsx", usage_text);
    lsp_did_open(
        &mut service,
        usage_uri.clone(),
        "typescriptreact",
