...
# reload trigger", config_text);
    ...
    assert!(
        diagnostics_reverted,
        "Diagnostics did not revert to valid disk state after closing document and reloading config"
    );
}
...
"#;
    ...
    assert!(found_error, "Expected error for INVALID_FIELD");
