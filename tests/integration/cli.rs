...
"#,
    ...
    // A cache the clean could not empty no longer decides the exit status, so
    // this asserts the outcome directly rather than forgiving a failure whose
    // stderr looked cache-shaped.
