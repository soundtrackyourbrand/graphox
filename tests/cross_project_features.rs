...
async fn test_cross_project_references_and_rename() {
    ...
    let request = Request::build("textDocument/references").id(1).params(serde_json::to_value(&params).unwrap()).finish();
    ...
    let request = Request::build("textDocument/rename").id(2).params(serde_json::to_value(&rename_params).unwrap()).finish();
    let response = service.call(request).await.unwrap().unwrap();
...
async fn test_unrelated_projects_rename_isolation() {
    ...
    let params = RenameParams {
        text_document_position: TextDocumentPositionParams {
            text_document: TextDocumentIdentifier { uri: p1_uri.clone() },
