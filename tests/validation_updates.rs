...
fn test_diagnostics_update_on_schema_change() {
    ...
    // Initially valid
    let diagnostics = doc.get_semantic_diagnostics(&schema_v1, &[], None);
    ...
    let diagnostics = doc.get_semantic_diagnostics(&schema_v2, &[], None);
    ...
    let diagnostics = doc_v2.get_semantic_diagnostics(&schema_v2, &[], None);
...
fn test_diagnostics_update_on_fragment_change() {
    ...
    // 1. Missing fragment
    let diagnostics = query_doc.get_semantic_diagnostics(&schema, &[], None);
    ...
    let diagnostics = query_doc.get_semantic_diagnostics(&schema, &fragments, None);
